...
app.post('/add-partner', (req, res) => {
  ...
  const newPartnerId = newPartner.partnerName.toLowerCase().replace(/\s+/g, '');
  partners[newPartnerId] = {
    thumbnailUrl: newPartner.partnerLogo,
    name: newPartner.partnerName,
    description: newPartner.partnerDesc,
    activity: newPartner.activity
  };
  res.status(201).send({ message: 'Partner added', partner: partners[newPartnerId] });
})

app.delete('/delete-partner/:name', (req, res) => {
  const partnerName = req.params.name.toLowerCase().replace(/\s+/g, '');
  if (partners[partnerName]) {
    delete partners[partnerName];
